...
void solve() {
    ...
    if (a == b) {
        LL ans = 0;
        ans = (ans + comb.binom(a + c - 1, a - 1) * comb.binom(b + d, b) % mod) % mod;
        ans = (ans + comb.binom(a + c, a) * comb.binom(b + d - 1, b - 1) % mod) % mod;
        std::cout << ans << "\n";
    } else {
        LL ans = 0;
        if (a > b) {
            ans = (ans + comb.binom(a + c - 1, a - 1) * comb.binom(a + d - 1, a - 1) % mod) % mod;
        } else {
            ans = (ans + comb.binom(b + c - 1, b - 1) * comb.binom(b + d - 1, b - 1) % mod) % mod;
        }
        std::cout << ans << "\n";
    }
